#!/usr/bin/env python3
"""
로컬 Docker PostgreSQL 데이터를 Supabase에 복원하는 스크립트
"""
        ...
        for i in range(0, total_rows, batch_size):
            ...
            for row in batch:
                for key, value in row.items():
                    if isinstance(value, str) and key == 'id' and len(value) == 36:
                        # UUID 형식인 경우 그대로 유지
                        pass
                    elif value is None:
                        row[key] = None
...
def main():
    """메인 함수"""
    print("🚀 로컬 Docker PostgreSQL 데이터를 Supabase에 복원 시작")
    print("=" * 70)
