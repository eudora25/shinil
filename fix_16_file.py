...
def fix_16_file():
    """16번 파일을 실제 API 응답 구조에 맞게 수정합니다."""
    
    try:
        # 엑셀 파일 로드
        wb = load_workbook('API_Files/16_직매매출_조회.xlsx')
        ws = wb.active
        
        print("=== 16번 파일을 실제 API 응답 구조로 수정 시작 ===")
        ...
        if output_section_row:
            ...
            new_output_params = [
                ...
                ["message", "STRING", "응답 메시지", "Y", "직매매출 조회 결과 메시지"],
                ["data[].id", "STRING", "매출 ID", "Y", "고유 식별자"],
                ["data[].pharmacy_code", "STRING", "약국 코드", "Y", "약국 고유 코드"],
                ["data[].pharmacy_name", "STRING", "약국명", "Y", "약국 이름"],
                ["data[].business_registration_number", "STRING", "사업자등록번호", "Y", "약국 사업자등록번호"],
                ["data[].address", "STRING", "주소", "Y", "약국 주소"],
                ["data[].standard_code", "STRING", "표준코드", "Y", "제품 표준코드"],
                ["data[].product_name", "STRING", "제품명", "Y", "제품 이름"],
                ["data[].sales_amount", "NUMERIC", "매출액", "Y", "매출 금액"],
                ["data[].sales_date", "STRING", "매출일", "Y", "매출 발생일"],
            ...
            # 파일 저장
            wb.save('API_Files/16_직매매출_조회.xlsx')
            print("16번 파일 수정이 완료되었습니다.")
