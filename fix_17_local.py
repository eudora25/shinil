...
def fix_17_local():
    """17번 파일을 로컬 API 응답 구조에 맞게 수정합니다."""
    
    try:
        # 엑셀 파일 로드
        wb = load_workbook('API_Files/17_실적정보_목록조회.xlsx')
        ws = wb.active
        
        print("=== 17번 파일을 로컬 API 응답 구조로 수정 시작 ===")
        ...
        if output_section_row:
            ...
            new_output_params = [
                ...
                ["data[].review_status", "STRING", "검토상태", "N", "검토 상태"],
                ["data[].review_action", "STRING", "검토 액션", "N", "검토 액션"],
                ["data[].updated_by", "STRING", "수정자", "N", "수정한 사용자 ID"],
                ["data[].commission_rate", "NUMERIC", "수수료율", "Y", "수수료율"],
                ["data[].review_completed_at", "STRING", "검토완료일시", "N", "검토 완료 시간"],
                ["data[].review_completed_by", "STRING", "검토완료자", "N", "검토 완료한 사용자 ID"]
            ]
            ...
            # 파일 저장
            wb.save('API_Files/17_실적정보_목록조회.xlsx')
            print("17번 파일 수정이 완료되었습니다.")
