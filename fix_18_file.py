...
def fix_18_file():
    """18번 파일을 실제 API 응답 구조에 맞게 수정합니다."""
    
    try:
        # 엑셀 파일 로드
        wb = load_workbook('API_Files/18_실적흡수율_정보.xlsx')
        ws = wb.active
        
        print("=== 18번 파일을 실제 API 응답 구조로 수정 시작 ===")
        ...
        if output_section_row:
            ...
            new_output_params = [
                ...
                ["message", "STRING", "응답 메시지", "Y", "실적흡수율 정보 조회 결과 메시지"],
                ["data[].id", "STRING", "실적 ID", "Y", "고유 식별자"],
                ["data[].created_at", "STRING", "생성일시", "Y", "ISO 8601 형식"],
                ["data[].settlement_month", "STRING", "정산월", "Y", "정산 월"],
                ["data[].company_id", "STRING", "업체 ID", "Y", "업체 고유 식별자"],
                ["data[].client_id", "STRING", "병원 ID", "Y", "병원 고유 식별자"],
                ["data[].product_id", "STRING", "제품 ID", "Y", "제품 고유 식별자"],
                ["data[].prescription_month", "STRING", "처방월", "Y", "처방 월"],
                ["data[].prescription_qty", "NUMERIC", "처방수량", "Y", "처방 수량"],
                ["data[].prescription_type", "STRING", "처방유형", "Y", "처방 유형"],
                ["data[].commission_rate", "NUMERIC", "수수료율", "Y", "수수료율"],
                ["data[].remarks", "STRING", "비고", "N", "추가 정보"],
                ["data[].registered_by", "STRING", "등록자", "Y", "등록한 사용자 ID"],
                ["data[].updated_at", "STRING", "수정일시", "N", "ISO 8601 형식"],
                ["data[].updated_by", "STRING", "수정자", "N", "수정한 사용자 ID"],
                ["data[].review_action", "STRING", "검토 액션", "N", "검토 액션"],
                ["data[].wholesale_revenue", "NUMERIC", "도매매출", "Y", "도매 매출액"],
                ["data[].direct_revenue", "NUMERIC", "직매매출", "Y", "직매 매출액"],
                ["data[].total_revenue", "NUMERIC", "총매출", "Y", "총 매출액"],
                ["data[].absorption_rate", "NUMERIC", "흡수율", "Y", "흡수율"],
                ["data[].analysis_time", "STRING", "분석시간", "N", "분석 시간"],
                ["data[].clients", "OBJECT", "병원 정보", "Y", "병원 상세 정보 객체"],
                ["data[].companies", "OBJECT", "업체 정보", "Y", "업체 상세 정보 객체"],
                ["data[].products", "OBJECT", "제품 정보", "Y", "제품 상세 정보 객체"],
            ...
            # 파일 저장
            wb.save('API_Files/18_실적흡수율_정보.xlsx')
            print("18번 파일 수정이 완료되었습니다.")
