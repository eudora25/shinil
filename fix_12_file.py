...
def fix_12_file():
    """12번 파일을 실제 API 응답 구조에 맞게 수정합니다."""
    
    try:
        # 엑셀 파일 로드
        wb = load_workbook('API_Files/12_병원업체_매핑정보.xlsx')
        ws = wb.active
        
        print("=== 12번 파일을 실제 API 응답 구조로 수정 시작 ===")
        ...
        if output_section_row:
            ...
            new_output_params = [
                ...
                ["message", "STRING", "응답 메시지", "Y", "병원업체 매핑 정보 조회 결과 메시지"],
                ["data[].id", "STRING", "매핑 ID", "Y", "고유 식별자"],
                ["data[].client_id", "STRING", "병원 ID", "Y", "병원 고유 식별자"],
                ["data[].company_id", "STRING", "업체 ID", "Y", "업체 고유 식별자"],
                ["data[].created_at", "STRING", "생성일시", "Y", "ISO 8601 형식"],
                ["data[].company_default_commission_grade", "STRING", "업체 기본 수수료 등급", "N", "업체의 기본 수수료 등급"],
            ...
            # 파일 저장
            wb.save('API_Files/12_병원업체_매핑정보.xlsx')
            print("12번 파일 수정이 완료되었습니다.")
