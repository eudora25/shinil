...
def check_11_local():
    """11번 파일을 로컬 API로 확인합니다."""
    
    try:
        print("=== 11번 파일 (병원약국_관계정보) 로컬 API 확인 ===")
        ...
        response = requests.get('http://localhost:3001/api/hospital-pharmacy-mappings', headers=headers)
        ...
        # 엑셀 파일에서 필드 추출
        wb = load_workbook('API_Files/11_병원약국_관계정보.xlsx')
        ws = wb.active
