// Express.js 라우터 형식으로 변경 (18_실적흡수율_정보.xlsx 형식에 맞춤)
...
// GET /api/performance-records-absorption - 실적흡수율 정보 조회 (18_실적흡수율_정보.xlsx 형식에 맞춤)
// Bearer Token 인증 필요
router.get('/', tokenValidationMiddleware, async (req, res) => {
  try {
    ...
    if (error) {
      console.error('Performance records absorption query error:', error)
      return res.status(500).json({
        success: false,
        message: '실적-흡수율 정보 목록 조회 중 오류가 발생했습니다.',
        error: error.message
      })
    }
