// Vercel 서버리스 함수 형식 (20_정산월_목록조회.xlsx 형식에 맞춤)
...
export default async function handler(req, res) {
  try {
    console.log('🚀 Settlement Months API 호출됨:', req.method, req.url)
    ...
    console.log('🔍 Settlement Months API - Supabase 설정 확인:')
    ...
    // 쿼리 파라미터 파싱
    const { page = 1, limit = 100, year, status } = req.query
    ...
    // 정렬 (최신순)
    query = query.order('settlement_month', { ascending: false })
