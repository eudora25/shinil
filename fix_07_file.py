...
def fix_07_file():
    """07번 파일을 실제 API 응답 구조에 맞게 수정합니다."""
    
    try:
        # 엑셀 파일 로드
        wb = load_workbook('API_Files/07_병원정보_조회.xlsx')
        ws = wb.active
        
        print("=== 07번 파일을 실제 API 응답 구조로 수정 시작 ===")
        ...
        if output_section_row:
            ...
            new_output_params = [
                ...
                ["message", "STRING", "응답 메시지", "Y", "병원 목록 조회 결과 메시지"],
                ["data[].id", "INTEGER", "병원 ID", "Y", "고유 식별자"],
                ["data[].client_code", "STRING", "고객 코드", "N", "고객 구분 코드"],
                ["data[].name", "STRING", "병원명", "Y", "병원 또는 의원명"],
                ["data[].business_registration_number", "STRING", "사업자등록번호", "N", "사업자등록번호"],
                ["data[].owner_name", "STRING", "대표자명", "N", "병원 대표자명"],
                ["data[].address", "STRING", "주소", "N", "병원 주소"],
                ...
                ["data[].remarks_settlement", "STRING", "정산 비고", "N", "정산 관련 비고"]
            ]
            ...
            # 파일 저장
            wb.save('API_Files/07_병원정보_조회.xlsx')
            print("✅ 07번 파일을 실제 API 응답 구조로 수정 완료!")
