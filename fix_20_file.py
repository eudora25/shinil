...
def fix_20_file():
    """20번 파일을 실제 API 응답 구조에 맞게 수정합니다."""
    ...
    try:
        ...
        print("=== 20번 파일을 실제 API 응답 구조로 수정 시작 ===")
        ...
        if output_section_row:
            ...
            new_output_params = [
                ...
                ["message", "STRING", "응답 메시지", "Y", "정산월 목록 조회 결과 메시지"],
                ["data[].id", "STRING", "정산월 ID", "Y", "고유 식별자"],
                ["data[].settlement_month", "STRING", "정산월", "Y", "정산 월"],
                ["data[].start_date", "STRING", "시작일", "Y", "정산 기간 시작일"],
                ["data[].end_date", "STRING", "종료일", "Y", "정산 기간 종료일"],
                ["data[].notice", "STRING", "공지사항", "N", "정산 관련 공지사항"],
                ["data[].status", "STRING", "상태", "Y", "정산 상태"],
                ...
                ["data[].notice_payment", "STRING", "지급공지", "N", "지급 관련 공지사항"],
            ...
            # 파일 저장
            wb.save('API_Files/20_정산월_목록조회.xlsx')
            print("20번 파일 수정이 완료되었습니다.")
