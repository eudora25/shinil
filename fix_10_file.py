...
def fix_10_file():
    """10번 파일을 실제 API 응답 구조에 맞게 수정합니다."""
    
    try:
        # 엑셀 파일 로드
        wb = load_workbook('API_Files/10_병원업체_관계정보.xlsx')
        ws = wb.active
        
        print("=== 10번 파일을 실제 API 응답 구조로 수정 시작 ===")
        ...
        if output_section_row:
            ...
            new_output_params = [
                ...
                ["message", "STRING", "응답 메시지", "Y", "병원-회사 매핑 목록 조회 결과 메시지"],
                ["data[].id", "INTEGER", "매핑 ID", "Y", "고유 식별자"],
                ["data[].client_id", "INTEGER", "병원 ID", "Y", "병원 식별자"],
                ["data[].company_id", "STRING", "회사 ID", "Y", "회사 식별자"],
                ["data[].created_at", "STRING", "생성일시", "Y", "ISO 8601 형식"],
                ["data[].company_default_commission_grade", "STRING", "기본 수수료 등급", "Y", "회사 기본 수수료 등급"],
                ["data[].modified_commission_grade", "STRING", "수정된 수수료 등급", "N", "수정된 수수료 등급"],
                ["data[].created_by", "STRING", "생성자", "N", "생성한 사용자 ID"],
                ["data[].clients", "OBJECT", "병원 정보", "Y", "병원 상세 정보"],
                ["data[].companies", "OBJECT", "회사 정보", "Y", "회사 상세 정보"],
                ["pagination", "OBJECT", "페이지네이션 정보", "Y", "페이지 정보"]
            ]
            ...
            # 파일 저장
            wb.save('API_Files/10_병원업체_관계정보.xlsx')
            print("✅ 10번 파일을 실제 API 응답 구조로 수정 완료!")
