-- API 로그를 위한 PostgreSQL 함수들 생성 (완전 초기화)
...
BEGIN
    ...
    FOR func_record IN 
        ...
        FROM pg_proc 
        WHERE proname = 'log_api_call' AND pronamespace = (SELECT oid FROM pg_namespace WHERE nspname = 'public')
    LOOP
        EXECUTE 'DROP FUNCTION IF EXISTS public.log_api_call(' || func_record.argtypes || ') CASCADE';
    END LOOP;
    ...
    FOR func_record IN 
        SELECT proname, oidvectortypes(proargtypes) as argtypes
        FROM pg_proc 
        WHERE proname = 'get_api_logs' AND pronamespace = (SELECT oid FROM pg_namespace WHERE nspname = 'public')
    LOOP
        EXECUTE 'DROP FUNCTION IF EXISTS public.get_api_logs(' || func_record.argtypes || ') CASCADE';
    END LOOP;
    
    -- get_api_logs_stats 함수들 삭제
    FOR func_record IN 
        SELECT proname, oidvectortypes(proargtypes) as argtypes
        FROM pg_proc 
        WHERE proname = 'get_api_logs_stats' AND pronamespace = (SELECT oid FROM pg_namespace WHERE nspname = 'public')
    LOOP
        EXECUTE 'DROP FUNCTION IF EXISTS public.get_api_logs_stats(' || func_record.argtypes || ') CASCADE';
    END LOOP;
END $$;

-- 3. API 로그 테이블 삭제 후 재생성
...
-- 6. API 로그 기록 함수
CREATE FUNCTION public.log_api_call(
...
-- 7. API 로그 조회 함수 (관리자용)
CREATE FUNCTION public.get_api_logs(
...
-- 8. API 로그 통계 함수 (관리자용)
CREATE FUNCTION public.get_api_logs_stats()
RETURNS TABLE (
    total_calls BIGINT,
