...
def fix_15_file():
    """15번 파일을 실제 API 응답 구조에 맞게 수정합니다."""
    
    try:
        # 엑셀 파일 로드
        wb = load_workbook('API_Files/15_도매매출_조회.xlsx')
        ws = wb.active
        
        print("=== 15번 파일을 실제 API 응답 구조로 수정 시작 ===")
        ...
        if output_section_row:
            ...
            new_output_params = [
                ...
                ["message", "STRING", "응답 메시지", "Y", "도매매출 조회 결과 메시지"],
            ...
            # 파일 저장
            wb.save('API_Files/15_도매매출_조회.xlsx')
            print("15번 파일 수정이 완료되었습니다.")
